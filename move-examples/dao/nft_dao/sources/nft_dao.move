...
///         b: Transfer EDS funds: from DAO account to the specified destination account.
...
module dao_platform::nft_dao {
    ...
    use endless_token::property_map::PropertyMap;
    use endless_token::property_map;
    use endless_token::token::{Self, TokenId, create_token_id_raw};
    ...
    public fun create_dao_and_get_dao_address(
        ...
        // register endless coin
        coin::register<EndlessCoin>(&res_signer);
    ...
    fun transfer_fund(res_acct: &signer, dst: address, amount: u64) {
        coin::transfer<EndlessCoin>(res_acct, dst, amount);
    }
    ...
    public fun create_one_token(
        ...
        create_token_script(
            ...
            amount,
            token_max,
            string::utf8(b"https://endless.dev"),
    ...
    #[test_only]
    use endless_token::token::create_token_script;
    #[test_only]
    use endless_framework::endless_account::transfer_coins;
    #[test_only]
    use endless_framework::endless_coin;
    use endless_token::token_transfers;
    ...
    #[test(endless_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_e2e_scenario(endless_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        ...
        let (burn_cap, mint_cap) = endless_coin::initialize_for_test(endless_framework);
        let coins = coin::mint(100, &mint_cap);
        coin::register<EndlessCoin>(creator);
        coin::register<EndlessCoin>(voter);
        coin::deposit(creator_addr, coins);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);

        // now resource account has a fund pool of 90 coins
        transfer_coins<EndlessCoin>(creator, res_acc, 90);
        ...
        assert!(coin::balance<EndlessCoin>(signer::address_of(voter)) == 45, 1);
    }

    #[test(endless_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_dao_offer_and_claim(endless_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(endless_framework);
    ...
    #[test(endless_framework = @0x1, admin = @0xdeaf)]
    public fun test_transferring_ownership_to_zero_address(endless_framework: &signer, admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(endless_framework);
        let old_addr = signer::address_of(admin);
        account::create_account_for_test(@0x1);
    ...
    public fun test_double_vote(endless_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(endless_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(endless_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_resolve_with_no_sufficient_votes(endless_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(endless_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    public fun test_resolve_earlier_than_ending_time(endless_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(endless_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(endless_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_execute_proposal(endless_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(endless_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(endless_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_veto_a_proposal(endless_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(endless_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(endless_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_set_dao_config(endless_framework: &signer, creator: &signer, voter: &signer) acquires DAO {
        timestamp::set_time_has_started_for_testing(endless_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(endless_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_admin_create_proposal_without_token(endless_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO, Proposals {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(endless_framework);
        let new_addr = signer::address_of(new_admin);
        let old_addr = signer::address_of(admin);
    ...
    #[test(endless_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_transfer_multiple_nfts(endless_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(endless_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
