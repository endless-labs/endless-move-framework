...
module coin_lucky_box::coin_lucky_box {
    ...
    struct Box has key, copy {
        ...
        random: bool,
        coin: object::Object<fungible_asset::Metadata>,
        amount: u128,
        count: u64,
    ...
    /// The coin which is packed in the box is not found
    const E_COIN_NOT_FOUND: u64 = 14;

    fun init_module(coin_lucky_box: &signer) {
        if (!exists<Funding>(@coin_lucky_box)) {
            let (_, resource_account_cap) = account::create_resource_account(coin_lucky_box, b"coin_lucky_box::Funding account seed");
            move_to(coin_lucky_box, Funding {
    ...
    public entry fun pack(caller: &signer, coin: address, amount: u128, count: u64, random: bool, expire: u64, unpackers: vector<u64>) acquires Funding {
        let unpacker_len = vector::length(&unpackers);
        assert!(unpacker_len <= MAX_UNPACKERS_COUNT, E_TOO_MANY_UNPACKERS);

        let box = pack_internal(caller, coin, amount, count, random, expire);
    ...
    public entry fun pack_with_merkle_tree(caller: &signer, coin: address, amount: u128, count: u64, random: bool, expire: u64, merkle_root: u64) acquires Funding {
        let box = pack_internal(caller, coin, amount, count, random, expire);
    ...
    public entry fun pack_with_guard(caller: &signer, coin: address, amount: u128, count: u64, random: bool, expire: u64, guard_pub_key: vector<u8>) acquires Funding {
        let box = pack_internal(caller, coin, amount, count, random, expire);
    ...
    fun pack_internal(caller: &signer, coin: address, amount: u128, count: u64, random: bool, expire: u64): signer acquires Funding {
        ...
        assert!(object::object_exists<fungible_asset::Metadata>(coin), E_COIN_NOT_FOUND);

        let coin = object::address_to_object<fungible_asset::Metadata>(coin);
        // Withdraw box amount from packer
        let amount_fa = primary_fungible_store::withdraw(caller, coin, amount);
        ...
        let gas_fa = primary_fungible_store::withdraw(caller, endless_coin::get_metadata(), (gas_fee as u128));
        // Deposit (box amount) + (precharge gas fee) to resource account
        primary_fungible_store::deposit(funding_address(), amount_fa);
        primary_fungible_store::deposit(funding_address(), gas_fa);
    ...
    fun unpack_internal(unpacker: address, id: address) acquires Box, Funding, Unpackers, MerkelRoot, GuardSigner {
        ...
        primary_fungible_store::transfer(funding_signer(), box.coin, unpacker, (amount as u128));
    ...
    fun refund_internal(id: address, box: &mut Box) acquires Funding, Unpackers, MerkelRoot, GuardSigner {
        ...
        // `box.gas_prepaid` is impossible to be zero here
        let gas = primary_fungible_store::withdraw(funding_signer(), endless_coin::get_metadata(), (box.gas_prepaid as u128));
        ...
        if (refund_amount > 0) {
            primary_fungible_store::transfer(funding_signer(), box.coin, packer, (refund_amount as u128));
        };
        if (refund_gas > 0) {
            let fa = fungible_asset::extract(&mut gas, (refund_gas as u128));
            primary_fungible_store::deposit(packer, fa);
        };
        primary_fungible_store::deposit(@coin_lucky_box, gas);
    ...
    inline fun funding_address(): address acquires Funding {
        let funding = borrow_global<Funding>(@coin_lucky_box);
