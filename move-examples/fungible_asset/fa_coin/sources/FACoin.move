...
module FACoin::fa_coin {
    ...
    public entry fun transfer(admin: &signer, from: address, to: address, amount: u128) acquires ManagedFungibleAsset {
        let asset = get_metadata();
    ...
    public entry fun burn(admin: &signer, from: address, amount: u128) acquires ManagedFungibleAsset {
        let asset = get_metadata();
    ...
    public fun withdraw(admin: &signer, amount: u128, from: address): FungibleAsset acquires ManagedFungibleAsset {
        let asset = get_metadata();
        let transfer_ref = &authorized_borrow_refs(admin, asset).transfer_ref;
        let from_wallet = primary_fungible_store::primary_store(from, asset);
