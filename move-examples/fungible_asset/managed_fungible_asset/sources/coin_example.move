...
module example_addr::coin_example {
    ...
    use endless_framework::fungible_asset::{Self, Metadata, FungibleAsset};
    ...
    public entry fun mint(admin: &signer, to: address, amount: u128) {
        managed_fungible_asset::mint_to_primary_stores(admin, get_metadata(), vector[to], vector[amount]);
    }

    /// Transfer as the owner of metadata object ignoring `frozen` field.
    public entry fun transfer(admin: &signer, from: address, to: address, amount: u128) {
    ...
    public entry fun burn(admin: &signer, from: address, amount: u128) {
    ...
    public fun withdraw(admin: &signer, from: address, amount: u128): FungibleAsset {
