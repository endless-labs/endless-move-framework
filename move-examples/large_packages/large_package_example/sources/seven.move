...
/// ### Account
///
/// - An **account** in the Endless blockchain is a container for an arbitrary number of [Move modules](#move-module) and [Move resources](#move-resources). This essentially means that the state of each [account](../concepts/accounts.md) is comprised of both code and data.
...
/// - An Endless address is a 64-character hex string, and sometimes it can be shortened by stripping leading 0s and prefixing `0x`. This makes a hex-encoded 32 byte Endless account address.
///
/// See [Accounts](../concepts/accounts.md) for more information.
///
/// ### API
///
/// - An **Application Programming Interface (API)(** is a set of protocols and tools that allow users to interact with Endless blockchain nodes and client networks via external applications. Endless offers a REST API for this purpose. See the [Endless API reference](https://endless.dev/nodes/endless-api-spec#/) documentation and [Use the Endless API](../integration/fullnode-rest-api.md) for more details.
///
/// ### EDS
///
...
/// ### Endless
///
/// **Endless** is a Layer 1 blockchain for everyone. It uses the Move programming language and launched its mainnet on 2022-10-17 to redefine the web3 user experience. The Endless blockchain is dedicated to creating better user experiences through increased speed, security, scalability, reliability and usability with low transaction costs.  The word "Endless" means "The People" in the Ohlone language. See the [Endless White Paper](../endless-white-paper/index.md) for more details.
///
/// ### EndlessBFT
///
...
///
/// ### Endless Name Service (ANS)
///
/// - The **Endless Name Service (ANS)** is a decentralized naming address service for the Endless blockchain. An Endless name is a human-readable *.EDS* domain name that is used in place of a public key, for example *love.EDS*.
/// - This service also allows users to register subdomain names in addition to the registered domain. Find out more at: [Endlessnames.com](https://www.endlessnames.com/)
///
/// ### Endless-core
///
/// **Endless-core** is the open source technology on which the Endless Payment Network runs. Endless-core contains software for
///
...
/// ### Endless Ecosystem
///
/// - **Endless ecosystem** refers to various components of the Endless blockchain network and their interactions.  The Endless ecosystem includes the community, community-driven projects, and events. See [Contribute to the Endless Ecosystem](../community/index.md) for all possible ways to join Endless.
///
/// ### Endless Explorer
///
/// - The **[Endless Explorer](https://explorer.endlesslabs.com/)** is an interface that helps users examine details of the Endless blockchain, including account information, validators, and transactions.
/// - The Endless Explorer help users validate their work in Endless wallets and other tools in the blockchain. Find more details at [Use the Endless Explorer](../guides/explore-endless.md).
///
/// ### Endless Framework
...
///
/// ### Endless Node
/// An **Endless node** is a peer entity of the Endless network that tracks the state of the Endless blockchain. There are two types of Endless nodes, [validators](#validator) and [fullnodes](#fullnode)).
///
/// ### Endless Protocol
///
...
/// - The Endless blockchain uses EndlessBFT, a consensus protocol based on [Jolteon](#Jolteon).
...
/// ### CLI
///
/// - **Command line interface** refers to the Endless CLI used for developing on the Endless blockchain, operating nodes, and debugging issues. Find out more at [Use the Endless CLI](../tools/endless-cli-tool/use-endless-cli.md).
///
/// ### Client
///
...
/// - In the Endless protocol, events provide evidence that the successful execution of a transaction resulted in a specific effect. The `DepositEvent` (in the above example) allows the recipient to confirm that a payment was received into their account.
...
///   - With the [Endless CLI](../tools/endless-cli-tool/use-endless-cli.md#fund-an-account-with-the-faucet).
///   - Through a wallet, such as Petra, Martian, or Pontem. You can find a full list [here](https://github.com/endless-foundation/ecosystem-projects#wallets).
...
/// - Follow the [Digital Asset Standards](../standards/index.md#digital-asset-standards) to create fungible assets on the Endless blockchain.
...
/// ### Fungible Token
///
/// - For TokenV1 (endless_token::token), a **fungible token** is a token that is interchangeable with other identical tokens (i.e., tokens that share the same `TokenId`). This means the tokens have the same `creator address`, `collection name`, `token name`, and `property version`.
/// - For TokenV2 (endless_token::token), a **fungible token** is a fungible asset with metadata object includes a TokenV2 resource.
///
/// ### Fungible Unit
///
/// - A **fungible unit** is an individual unit of a fungible asset. These units are identical and interchangeable without any loss in value. For example, each Vein (the smallest unit of EDS) is a fungible unit.
///
/// ## G
///
/// ### Gas
...
/// ### Indexer
///
/// - **[Indexer](../integration/indexing.md)** is the component of Endless that retrieves, processes, and efficiently stores raw data in the database to provide speedy access to the Endless blockchain state.
///
/// ## L
///
/// ### Leader
...
/// ### Merkle Accumulator
///
/// - The **[Merkle Accumulator](https://www.usenix.org/legacy/event/sec09/tech/full_papers/crosby.pdf)** is an _append-only_ Merkle tree that the Endless blockchain uses to store the ledger.
...
/// - Move is a safe and secure programming language for web3 that emphasizes access control and scarcity. It is the programming language used to build the Endless blockchain. You can read more about it in [Move on Endless](../move/move-on-endless.md).
///
/// ### Move Bytecode
///
...
/// - An Endless node consists of logical components. [Mempool](#mempool), [consensus](#consensus), and the [virtual machine](#virtual-machine) are examples of node components.
///
/// ### Nonce
///
...
///
/// See [Resource accounts](../move/move-on-endless/resource-accounts.md) for instructions on use.
///
/// ### REST Service
///
/// - The **REST Service** component is the external interface of a Endless node. Any incoming client request, such as submitted transactions or queries, must first go through the REST Service. A client needs to go through the REST Service component to access storage or any other component in the system. This filters requests and protects the system.
...
/// ### SDKs
///
/// - Endless **software development kits (SDKs)** are sets of tools that enable a developer to quickly create a custom app on the Endless platform. Find out more at [Use the Endless SDKs](../sdks/index.md).
///
/// ### Sequence Number
///
...
/// ### State Root Hash
///
/// - **State root hash** is a [Merkle hash](https://en.wikipedia.org/wiki/Merkle_tree) over all keys and values the state of the Endless blockchain at a given version.
///
/// ## T
///
/// ### Table
///
/// - A [**table**](https://github.com/endless-labs/endless-core/blob/main/endless-move/framework/endless-stdlib/doc/table.md) implements the Table type and in Endless is used to store information as key-value data within an account at large scale.
///
/// See [`table.move`](https://github.com/endless-labs/endless-core/blob/main/endless-move/framework/endless-stdlib/sources/table.move) for the associated Endless source file.
///
/// ### Testnet
///
...
/// - The transaction script is an arbitrary program that interacts with resources published in the global storage of the Endless blockchain by calling the procedures of a module. It encodes the logic for a transaction.
...
///
/// To see example uses of transaction scripts, follow [Move scripts](../move/move-on-endless/move-scripts.md) and the [Your First Multisig](../tutorials/first-multisig.md) tutorial.
///
/// ## V
///
/// ### Validator
...
/// - The Endless blockchain doesn't have an explicit notion of a block &mdash; it only uses blocks for batching and executing transactions.
