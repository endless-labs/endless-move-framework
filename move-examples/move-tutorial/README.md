---
title: "Endless Move Tutorial"
slug: "move-tutorial"
---

# Move Tutorial

Welcome to the Endless Move Tutorial! This is the original Move language and tools tutorial, independent of a particular network, adapted to use Endless tooling. Here you will learn basic usage of the Move language and tools for compiling, testing, and verifying Move.

This tutorial does not teach you how to use the existing [Endless Move frameworks](https://endless.dev/reference/move) or how to run code on the Endless network. See the [Endless developer documentation](../../../developer-docs-site/docs/tutorials/index.md) for those instructions.
...
You also need a recent version of the [Endless CLI](../../../developer-docs-site/docs/tools/endless-cli/install-cli/index.md). This tutorial is written using the following version:
...
If you want to use an IDE for working through the examples, we recommend CLion/IntelliJ, which has good support for Endless Move.
...
Let us make sure it builds! This can be done with the `endless move compile` command from within the package directory ([`step_1/basic_coin`](./step_1/basic_coin/)):
...
* You can create an empty Move package by calling:
    ```bash
    endless move init --name <pkg_name>
    ```
...
You can run the tests with the `endless move test` command:
...
* Change the assertion to `11` so that the test fails. Find a flag that you can
  pass to the `endless move test` command that will dump the global state when
...
* Find a flag that allows you to gather test coverage information, and
  then experiment with using the `endless move coverage` command to examine
...
> NOTE: The coin and balance interfaces are for illustration of Move concepts only. Endless uses a different, richer [coin type](https://github.com/endless-labs/endless-core/blob/main/endless-move/framework/endless-framework/doc/coin.md#0x1_coin) contained in the Endless framework.
...
[`error` module]: https://github.com/endless-labs/endless-core/blob/main/endless-move/framework/move-stdlib/sources/error.move
...
To get started, run the `endless move test` command in the [`step_5/basic_coin`](./step_5/basic_coin) directory:
...
Read more about [phantom type](../../../developer-docs-site/docs/move/book/generics.md#phantom-type-parameters) parameters in the Endless Move Book.
</details>

## Step 7:  Use the Move Prover
    
> NOTE: Before running the Move Prover, ensure that the [Move Prover](../../../developer-docs-site/docs/tools/endless-cli/install-cli/install-move-prover.md) and associated tools are installed.
