...
///
/// WARNING #2: This code makes a STRONG assumption that the Endless clock and the drand clock are synchronized.
/// In practice, the Endless clock could be lagging behind. As an example, even though the current time is Friday, July
...
/// contracts account for any drift between the Endless clock and the drand clock. In this example, this can be done by
...
module drand::lottery {
    ...
    public fun get_ticket_price(): u128 { TICKET_PRICE }
    ...
    public entry fun buy_a_ticket(user: &signer) acquires Lottery {
        ...
        endless_coin::transfer(user, rsrc_acc_addr, TICKET_PRICE);
    ...
    public entry fun close_lottery(drand_signed_bytes: vector<u8>) acquires Lottery {
        ...
        let balance = endless_coin::balance(rsrc_acc_addr);
