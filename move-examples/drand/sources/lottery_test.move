module drand::lottery_test {
    ...
    #[test_only]
    use endless_framework::fungible_asset::{Self, MintRef};
    #[test_only]
    ...
    use std::debug;
    #[test_only]
    use endless_std::crypto_algebra::enable_cryptography_algebra_natives;

    #[test_only]
    fun give_coins(mint_ref: &MintRef, to: &signer) {
        let to_addr = signer::address_of(to);
        if (!account::exists_at(to_addr)) {
            account::create_account_for_test(to_addr);
        };

        primary_fungible_store::mint(mint_ref, to_addr, lottery::get_ticket_price());
    }

    #[test(myself = @drand, fx = @endless_framework, u1 = @0xA001, u2 = @0xA002, u3 = @0xA003, u4 = @0xA004)]
    fun test_lottery(
        ...
        let (mint_ref, _, _) = endless_coin::initialize_for_test(&fx);
        ...
        while(!vector::is_empty(&vec_signed_bytes)) {
            ...
            // Create fake coins for users participating in lottery & initialize endless_framework
            give_coins(&mint_ref, &u1);
            give_coins(&mint_ref, &u2);
            give_coins(&mint_ref, &u3);
            give_coins(&mint_ref, &u4);
