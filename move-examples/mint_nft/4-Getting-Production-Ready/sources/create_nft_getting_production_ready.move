...
/// We use signature validation for many reasons. For example, in endless_framework::account::rotate_authentication_key(),
...
/// For more information about events, see: https://endless.dev/concepts/events/.
///
/// Move unit tests
/// We added a few unit tests to make sure that our code is working as expected. For more information on how to write
/// Move unit tests, see: https://endless.dev/move/book/unit-testing
///
...
/// Run the following command in directory `endless-core/endless-move/move-examples/mint_nft/4-Getting-Production-Ready`.
/// - 2.b Run the following CLI command to publish the module under a resource account.
/// endless move create-resource-account-and-publish-package --seed [seed] --address-name mint_nft --profile default --named-addresses source_addr=[default account's address]
///
/// 3. Call `mint_event_ticket()` with a valid signature to mint a token.
/// - 3.a Generate a keypair to validate the signature against.
/// endless key generate --key-type ed25519 --output-file output.key
/// - 3.b Update the public key stored within `ModuleData`.
/// endless move run --function-id [resource account's address]::create_nft_getting_production_ready::set_public_key --args hex:[public key we just generated] --profile admin
/// (To find the public key we just generated, run `nano output.key.pub`.)
/// example output:
/*
4-Getting-Production-Ready % endless move run --function-id f634035fea40e23c5ed8817f7e996d96372dd5dbd64e853fb3c108817d92dcb4::create_nft_getting_production_ready::set_public_key --args hex:5C0637A3865FCA80550502BC30C8E7B4CCA7C8AB3B4FFECEFC8C43F7D0D44DEE --profile admin
Do you want to submit a transaction for a range of [35200 - 52800] Veins at a gas unit price of 100 Veins? [yes/no] >
yes
{
  "Result": {
...
///  Open up file `endless-core/endless-move/e2e-move-tests/src/tests/mint_nft.rs`.
...
///  You can find the `admin_private_key` by running `nano output.key`, and the `receiver_account_sequence_number` by looking up the receiver's address on the Endless Explorer under tab `Info`.
///  Make sure you're in the right directory.
///  Run the following command in directory `endless-core/endless-move/e2e-move-tests`
...
/// endless move run --function-id [resource account's address]::create_nft_getting_production_ready::mint_event_ticket --args hex:[signature generated in last step] --profile nft-receiver
/// example output:
/*
4-Getting-Production-Ready % endless move run --function-id f634035fea40e23c5ed8817f7e996d96372dd5dbd64e853fb3c108817d92dcb4::create_nft_getting_production_ready::mint_event_ticket --args hex:fc833512ad1c575850569d14f5e434b929a19eb491c08df9f6b91584a13551bdb95830081a429f148fddcb9ba201cf72a357957849046da0d60675ed034f580f --profile nft-receiver
Do you want to submit a transaction for a range of [600800 - 901200] Veins at a gas unit price of 100 Veins? [yes/no] >
yes
{
  "Result": {
...
module mint_nft::create_nft_getting_production_ready {
    ...
    use endless_token::token::{Self, TokenDataId};
    use endless_framework::resource_account;
    #[test_only]
    use endless_framework::account::create_account_for_test;
    use endless_std::ed25519::ValidatedPublicKey;
    ...
    public entry fun test_happy_path(
        ...
        set_up_test(origin_account, &resource_account, &admin_pk, endless_framework, &nft_receiver, 10);
    ...
    public entry fun test_minting_expired(
        ...
        let (admin_sk, admin_pk) = ed25519::generate_keys();
        set_up_test(origin_account, &resource_account, &admin_pk, endless_framework, &nft_receiver, 100000000001);
        let receiver_addr = signer::address_of(&nft_receiver);
        let proof_challenge = MintProofChallenge {
            receiver_account_sequence_number: account::get_sequence_number(receiver_addr),
            receiver_account_address: receiver_addr,
