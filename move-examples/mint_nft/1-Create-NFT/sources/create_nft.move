...
/// 1. Create an account.
/// endless init (this will create a default account)
///
/// 2. Publish the module.
/// - 2.a Make sure you're in the right directory.
/// Run the following command in directory `endless-core/endless-move/move-examples/mint_nft/1-Create-NFT`.
/// - 2.b Run the following CLI command to publish the module.
/// endless move publish --named-addresses mint_nft=[default account's address]
/// (If you don't know the default account's address, run `nano ~/.endless/config.yaml` to see all addresses.)
///
/// example output:
    /*
    1-Create-NFT % endless move publish --named-addresses mint_nft=a911e7374107ad434bbc5369289cf5855c3b1a2938a6bfce0776c1d296271cde
    ...
    Do you want to submit a transaction for a range of [1164400 - 1746600] Veins at a gas unit price of 100 Veins? [yes/no] >
    yes
    {
      "Result": {
    ...
    */
/// - 2.c Check the module we just published on the Endless Explorer.
/// Go to https://explorer.endlesslabs.com/. At the top right of the screen, select the network you used (devnet, testnet, etc.).
